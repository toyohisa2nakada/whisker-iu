...
describe("Arrays", () => {
    ...
    test("Add array to list", () => {
        Arrays_1.default.addAll(array, [4, 5]);
        expect(array.length).toBe(5);
    });
    test("Is array empty", () => {
        const empty = [];
        expect(Arrays_1.default.isEmpty(array)).toBeFalsy();
        expect(Arrays_1.default.isEmpty(empty)).toBeTruthy();
    });
    ...
    test("Test FindElement", () => {
        expect(Arrays_1.default.findElement(array, 2)).toEqual(1);
    });
    test("Clear array", () => {
        Arrays_1.default.clear(array);
        expect(array.length).toBe(0);
    });
    test("Clone array", () => {
        const clone = Arrays_1.default.clone(array);
        Arrays_1.default.clear(array);
        expect(clone.length).toBe(3);
    });
    test("Remove element from array", () => {
        Arrays_1.default.remove(array, 2);
        expect(array[1]).toBe(3);
    });
    test("Remove element from array given an index", () => {
        Arrays_1.default.removeAt(array, 1);
        expect(array[1]).toBe(3);
    });
    test("Remove all elements from source array", () => {
        const source = [1, 2, 3, 4, 5, 6];
        const remove = [2, 4, 6];
        expect(Arrays_1.default.removeAll(source, remove)).toEqual([1, 3, 5]);
    });
    test("Remove all elements from source array with empty remove array", () => {
        const source = [1, 2, 3, 4, 5, 6];
        const remove = [];
        expect(Arrays_1.default.removeAll(source, remove)).toEqual([1, 2, 3, 4, 5, 6]);
    });
    test("Remove all elements from source array with identical remove array", () => {
        const source = [1, 2, 3, 4, 5, 6];
        expect(Arrays_1.default.removeAll(source, source)).toEqual([]);
    });
    test("Remove all elements from source array with duplicates in source array", () => {
        const source = [1, 2, 2, 3];
        const remove = [2];
        expect(Arrays_1.default.removeAll(source, remove)).toEqual([1, 3]);
    });
    test("Get distinct array", () => {
        const array = [1, 2, 3, 3];
        const distinct = Arrays_1.default.distinct(array);
    ...
    test("Shuffle array", () => {
        ...
        for (let i = 0; i < 100; i++) {
            Arrays_1.default.shuffle(array);
    ...
    test("Chunk array", () => {
        ...
        const chunkArray = Arrays_1.default.chunk(array, 3);
    ...
    test("Distinct objects", () => {
        const array = [new ClickStageEvent_1.ClickStageEvent(), new ClickStageEvent_1.ClickStageEvent(), new ClickStageEvent_1.ClickStageEvent()];
        const distinct = Arrays_1.default.distinctObjects(array);
        expect(distinct.length).toBe(1);
    });
    test("Distinct objects by custom defined comparator", () => {
        const comparator = (a, b) => a.stringIdentifier() === b.stringIdentifier();
        const array = [new ClickStageEvent_1.ClickStageEvent(), new MouseDownEvent_1.MouseDownEvent(false), new MouseDownEvent_1.MouseDownEvent(false),
            new MouseMoveEvent_1.MouseMoveEvent(2, 1), new MouseMoveEvent_1.MouseMoveEvent(2, 1), new MouseMoveEvent_1.MouseMoveEvent(10, 10)];
        const distinct = Arrays_1.default.distinctByComparator(array, comparator);
        expect(distinct.length).toBe(3);
    ...
    test("Range function stepSize of 1", () => {
        const rangeArray = Arrays_1.default.range(0, 10, 1);
        expect(rangeArray.reduce((a, b) => a + b, 0)).toBe(45);
    });
    test("Range function stepSize of 100", () => {
        const rangeArray = Arrays_1.default.range(0, 850, 100);
    ...
    test("Create random Array in range", () => {
        ...
        const maxValue = 30;
        const length = 100;
        const array = Arrays_1.default.getRandomArray(minValue, maxValue, length);
    ...
    test("Subtract two arrays, e.g. vectors of equal size", () => {
        ...
        expect(Arrays_1.default.subtract(array, subtrahend)).toStrictEqual(expected);
    });
    ...
    test("Scalar multiplication", () => {
        ...
        expect(Arrays_1.default.scalarProduct(array, 3)).toStrictEqual(expected);
    });
    test("Arrays.last returns last element from array", () => {
        const actual = Arrays_1.default.last(array);
