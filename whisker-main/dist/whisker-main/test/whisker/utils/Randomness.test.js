...
describe("Randomness", () => {
    test("Set initial seed as number type", () => {
        ...
        expect(Randomness_1.Randomness.getInitialRNGSeed()).toBe(seed);
    });
    test("Set initial seed as string type", () => {
        const seed = "5";
        Randomness_1.Randomness.setInitialSeeds(seed);
        expect(Randomness_1.Randomness.getInitialRNGSeed()).toBe(5);
        expect(Randomness_1.Randomness.getInstance()).toBeInstanceOf(Randomness_1.Randomness);
    });
    test("Set initial seed as string", () => {
        const seed = "whisker";
        Randomness_1.Randomness.setInitialSeeds(seed);
        expect(Randomness_1.Randomness.getInitialRNGSeed()).toBeGreaterThan(0);
        expect(Randomness_1.Randomness.getInstance()).toBeInstanceOf(Randomness_1.Randomness);
    });
    test("Set initial seed as empty string", () => {
        const seed = "";
        Randomness_1.Randomness.setInitialSeeds(seed);
        expect(Date.now() - Randomness_1.Randomness.getInitialRNGSeed()).toBeLessThan(1000);
        expect(Randomness_1.Randomness.getInstance()).toBeInstanceOf(Randomness_1.Randomness);
    });
    ...
    test("Different seed, different sequence", () => {
        const random = Randomness_1.Randomness.getInstance();
        Randomness_1.Randomness.setInitialSeeds(0);
        const sequence1 = [];
        for (let i = 0; i < 100; i++) {
            sequence1.push(random.nextInt(0, 100));
        }
        Randomness_1.Randomness.setInitialSeeds(42);
        const sequence2 = [];
        for (let i = 0; i < 100; i++) {
