...
class CoverageGenerator {
     ...
     */
    static getCoverage() {
        return new Coverage(CoverageGenerator.getCoveredBlockIdsPerSprite(), CoverageGenerator.getBlockIdsPerSprite(), CoverageGenerator.getBlockDescriptions(), CoverageGenerator.getCoveredBlockIdsPerTest(), CoverageGenerator.getCoveredBlockIdsPerAssertion());
    }
    /**
    ...
    static traceExecution(thread) {
        ...
        const inputContent = Object.keys(input).map((key) => cloneDeep(target.blocks.getBlock(input[key].block)));
        ...
        return {
            ...
            target: {
                ...
                touching: otherSpritesName.filter(x => (x !== target.getName() && target.isTouchingSprite(x))),
