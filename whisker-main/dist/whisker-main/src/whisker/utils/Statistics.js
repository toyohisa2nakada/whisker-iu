...
class Statistics {
     ...
     */
    static silvermanRuleOfThumb(samples) {
     ...
     */
    static levenshteinDistanceEventsChunks(a, b, chunkSize) {
        const sourceChunks = Arrays_1.default.chunk(a, chunkSize);
        const targetChunks = Arrays_1.default.chunk(b, chunkSize);
        let distances;
        if (sourceChunks.length > targetChunks.length) {
            distances = sourceChunks.map((value, index) => this.levenshteinDistanceEvents(value, targetChunks[index]));
        }
        else {
            distances = targetChunks.map((value, index) => this.levenshteinDistanceEvents(value, sourceChunks[index]));
        }
     ...
     */
    static levenshteinDistanceEvents(a, b) {
        const aStr = a === null || a === void 0 ? void 0 : a.map((event) => event.toJavaScript());
        const bStr = b === null || b === void 0 ? void 0 : b.map((event) => event.toJavaScript());
