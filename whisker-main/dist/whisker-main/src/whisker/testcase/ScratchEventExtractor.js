...
exports.ScratchEventExtractor = void 0;
...
class ScratchEventExtractor {
    constructor(vm) {
        this._knownColors = new Map();
     ...
     */
    traverseBlocks(target, block, foundEvents) {
        while (block) {
    ...
    static _getColorFromBlock(target, block, key) {
        const str = target.blocks.getBlock(block.inputs[key].block).fields.COLOUR.value;
        const arr = cast_1.default.toRgbColorList(str);
        return { str, arr };
    }
    _handleTouchingColor(target, block, events) {
        ...
        if (!target.isTouchingColor(color.arr)) {
            ...
            if (result.colorFound) {
                ...
                events.push(new DragSpriteEvent_1.DragSpriteEvent(target, x, y));
            }
        }
    }
    _handleColorTouchingColor(target, block, events) {
        /*
        ...
        if (!target.colorIsTouchingColor(ownColor.arr, targetColor.arr)) {
            ...
            if (ownColorQuery.colorFound) {
                ...
                if (targetColorQuery.colorFound) {
                    ...
                    events.push(new DragSpriteEvent_1.DragSpriteEvent(target, targetX + offsetX, targetY + offsetY));
                }
            }
            else {
        ...
        switch (target.blocks.getOpcode(block)) {
            ...
            case 'sensing_touchingobject': {
                ...
                // Target senses another sprite
                    ...
                    let sensedRenderedTarget = Container_1.Container.vmWrapper.getTargetBySpriteName(value);
                    ...
                    if (target.visible &&
                        ...
                        eventList.push(new DragSpriteEvent_1.DragSpriteEvent(target, sensedRenderedTarget.x, sensedRenderedTarget.y));
                    }
                }
                break;
            }
            ...
            case 'sensing_mousedown': {
                ...
                eventList.push(new MouseDownEvent_1.MouseDownEvent(!isMouseDown));
                break;
            }
            case 'sensing_askandwait':
                // Type text
                if (Container_1.Container.vmWrapper.isQuestionAsked()) {
     ...
     */
    extractAvailableTextSnippets(vm) {
    ...
    static _comparesVariableOrAnswer(vm) {
        for (const target of vm.runtime.targets) {
            if ('blocks' in target) {
                for (const blockId of Object.keys(target.blocks._blocks)) {
    ...
    static _randomText(length) {
        ...
        for (let i = 0; i < length; i++) {
            answer += chars.charAt(Randomness_1.Randomness.getInstance().nextInt(0, chars.length - 1));
        }
        return answer;
    }
    static _extractAvailableTextSnippets(target, block) {
    ...
    _getTypeTextEvents() {
        ...
        for (let i = 0; i < length; i++) {
            typeTextEventList.push(new TypeTextEvent_1.TypeTextEvent(this.availableTextSnippets[i]));
        }
    ...
    static _searchForMouseEvent(target, block) {
        if (typeof block.opcode === 'undefined') {
            return false;
        }
        switch (target.blocks.getOpcode(block)) {
    ...
    static _hasEvents(target, block) {
        if (typeof block.opcode === 'undefined') {
            return false;
        }
        switch (target.blocks.getOpcode(block)) {
     ...
     */
    _findColorOnCanvas(sprite, color) {
     ...
     * @return the color query result
     */
    _findColorOnSprite(sprite, color) {
        const renderer = sprite.runtime.renderer;
    ...
    _fuzzyFindColor(start, touchables, { arr, str }, bounds, renderer) {
        function isColorMatching(other) {
    ...
    static *_points({ x: startX, y: startY }, bounds, space = 10) {
         ...
         * @return {boolean} true iff within bounds
         */
        function isWithinVerticalBounds(y) {
...
exports.ScratchEventExtractor = ScratchEventExtractor;
_a = ScratchEventExtractor;
ScratchEventExtractor._fixedStrings = ["0", "10", "Hello", _a._randomText(3)];
/**
